...
const getEnabledProviders = () => {
  return Object.keys(oauthConfig).filter(provider => 
    oauthConfig[provider].clientID && oauthConfig[provider].clientSecret
  );
};

// Session configuration for OAuth
const sessionConfig = {
  secret: process.env.OAUTH_SESSION_SECRET || process.env.JWT_SECRET || 'fallback-secret-change-in-production',
  resave: false,
