...
export function getSlowestEndpoints(limit = 10) {
  ...
  return endpoints.sort((a, b) => b.averageTime - a.averageTime).slice(0, limit);
}
...
export function exportTimingData(format = 'json') {
  const data = {
    ...
    summary: {
      totalRequests: Array.from(endpointStats.values()).reduce(
        (sum, stats) => sum + stats.totalRequests,
        0
      ),
      totalEndpoints: endpointStats.size,
      slowRequests: Array.from(endpointStats.values()).reduce(
        (sum, stats) => sum + stats.slowRequests,
        0
      ),
    },
  ...
  return Array.from(endpointStats.entries()).map(([endpoint, stats]) => ({
    ...
    recentAverage:
      stats.recentTimes.length > 0
        ? Math.round(
            stats.recentTimes.slice(-10).reduce((a, b) => a + b, 0) /
              Math.min(10, stats.recentTimes.length)
          )
        : 0,
  }));
}
