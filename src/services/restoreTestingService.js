...
class RestoreTestingService {
  ...
  async validateRestoredData(restoreResult) {
    try {
      ...
      const stagingConnection = mongoose.createConnection(restoreResult.stagingUri);
  ...
  async generateQuarterlyReport(testResult) {
    const report = {
      ...
      summary: {
        ...
        successfulRestores:
          (testResult.fullRestoreResult.success ? 1 : 0) +
          (testResult.incrementalRestoreResults.success ? 1 : 0),
        failedRestores:
          (testResult.fullRestoreResult.success ? 0 : 1) +
