  ...
  getTrashItems: async (req, res) => {
    try {
      ...
      if (resourceType && TRASH_MODELS[resourceType]) {
        ...
        const total = await model.countDocuments(query).setOptions({ includeDeleted: true });
      ...
      for (const [type, { model, displayName }] of Object.entries(TRASH_MODELS)) {
        try {
          ...
          const count = await model.countDocuments(query).setOptions({ includeDeleted: true });
      ...
      for (const [type, { model, displayName }] of Object.entries(TRASH_MODELS)) {
        try {
          ...
          if (count > 0) {
            ...
            if (
              !stats.oldestItem ||
              new Date(oldest.deletedAt) < new Date(stats.oldestItem.deletedAt)
            ) {
              stats.oldestItem = { ...oldest, resourceType: type };
            }
            if (
              !stats.newestItem ||
              new Date(newest.deletedAt) > new Date(stats.newestItem.deletedAt)
            ) {
