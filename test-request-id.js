...
console.log(
  `   ✅ Valid UUID format: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(id1) ? 'PASS' : 'FAIL'}`
);
...
correlationIdMiddleware(mockReq, mockRes, () => {
  ...
  console.log(
    `   ✅ IDs match: ${mockReq.requestId === mockRes.headers['x-request-id'] ? 'PASS' : 'FAIL'}`
  );
...
correlationIdMiddleware(mockReq2, mockRes2, () => {
  console.log(
    `   ✅ Existing ID preserved: ${mockReq2.requestId === 'test-existing-id-123' ? 'PASS' : 'FAIL'}`
  );
  console.log(
    `   ✅ Response uses existing: ${mockRes2.headers['x-request-id'] === 'test-existing-id-123' ? 'PASS' : 'FAIL'}`
  );
});
...
console.log(
  `   ✅ Header added to options: ${enhancedOptions.headers['X-Request-ID'] === testRequestId ? 'PASS' : 'FAIL'}`
);
console.log(
  `   ✅ Original headers preserved: ${enhancedOptions.headers['Content-Type'] === 'application/json' ? 'PASS' : 'FAIL'}`
);
